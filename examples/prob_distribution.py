...
class StackedProbDistribution(VGroup):
    ...
    def reposition_labels(self):
        ...
        self.labels.set_height(self.bars.get_height()
                               * self.label_height_ratio)
        self.labels.move_to(self.bars)
        for label, bar in zip(self.labels, self.bars):
            label.match_x(bar)
            fill_opacity = float(label.get_width() <
                                 bar.get_width() * self.label_width_ratio)
...
class DynamicInterval(UnitInterval):
    def __init__(
        ...
        **kwargs
        ...
        num_decimal_places = max(
            int(np.round(-np.log(step) / np.log(subdivisions))), 0)
    ...
    def update_opacity_from_width(self):
        ...
        opacity = clip(inverse_interpolate(
            w_min, w_max, self.get_width()), 0, 1)

...
class TriangleSquareRotation(Scene):
    def construct(self):
        ...
        vertex_a = np.array([0, 0, 0])           # left point (rotation center)
        ...
        # First rotation about vertex_a
        self.play(
            Rotate(triangle, angle=210 * DEGREES, about_point=vertex_a, run_time=1)
        )
        ...
        self.add(trace)
        self.play(
            Rotate(triangle, angle=210 * DEGREES, about_point=vertex_b, run_time=1)
        )
        self.play(FadeOut(top_dot), run_time=0.5)
        triangle.remove(top_dot)
